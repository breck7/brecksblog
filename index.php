...
#right_column a:hover {background: #f9f9aa;}",
    ...
    public function controller()
    {
        ...
        else // Display Homepage
        {
            ...
            {
                $all_posts .= "<h2><a href=\"".$this->prettyUrl($post['Title'])."\">{$post['Title']}</a></h2>";
                $all_posts .= "<div>".$this->format_post(substr(strip_tags($post['Essay']),0,150));
                $all_posts .= "<a href=\"".$this->prettyUrl($post['Title'])."\">...continue to full essay.</a>";
                $all_posts .= "<br><br><div class=\"dateposted\">Posted ".date("m/d/Y", $key )."</div></div><br><br>";
            }
            $this->displayPage(BLOG_TITLE, BLOG_DESCRIPTION, $all_posts); 
        }
    }
